...
from gci_utils import insert_trans_gci, seconds2samples, samples2seconds, sync_gci_to_samp_peak, create_gci
...
class Scorer(object):
    ...
    def identification_accuracy_error(self):
        ...
        return float(np.array(self._errors).std())
...
def predict_gci(estimator, X):
    ...
    logger.debug('# GCIs predicted: {}'.format(len(y)))
    return y


# Evaluate pairs of pitch-mark objects
def evaluate(gci_pairs, abs_dist=.00025, rel_dist=10, min_t0=.02, sync_le=0., sync_ri=0.):
    ...
    gscorer_rel = Scorer(dist_threshold=rel_dist, scoring='ida', min_t0=min_t0, sync_le=sync_le,
                         sync_ri=sync_ri)
    gscorer_abs = Scorer(dist_threshold=abs_dist, scoring='idr', sync_le=sync_le, sync_ri=sync_ri)

    for ref_pm, tst_pm in gci_pairs:
        # Read GCIs from pm file if not PM objects inputted
        if not isinstance(ref_pm, Pm):
            ref_pm = Pm(ref_pm)
        if not isinstance(tst_pm, Pm):
            tst_pm = Pm(tst_pm)

        logger.debug('Comparing: {} ({} GCIs) vs. {} ({} GCIs)'.format(ref_pm.source, len(ref_pm), tst_pm.source, len(tst_pm)))
        
        # WA: If test sentence is empty (no GCI present) => add 2 fake GCIs to enable comparison
        if len(tst_pm) == 0:
            logger.debug('Test sequence is empty => adding 2 fake GCIs')
            tst_pm = create_gci([0.001, 0.002], tst_pm.source)

        # Local scorers (scoring for a single GCI pair comparison)
        lscorer_rel = Scorer(dist_threshold=rel_dist, scoring='ida', min_t0=min_t0, sync_le=sync_le,
                             sync_ri=sync_ri)
        lscorer_abs = Scorer(dist_threshold=abs_dist, scoring='idr', sync_le=sync_le, sync_ri=sync_ri)

        # Compare two corresponding GCI files and accumulate the comparison measures
        gscorer_rel.accumulate_cmps(lscorer_rel.compare_and_accumulate(ref_pm, tst_pm))
        gscorer_abs.accumulate_cmps(lscorer_abs.compare_and_accumulate(ref_pm, tst_pm))

        logger.debug('{:20}: ACC{} = {:.2%}'.format(ref_pm.source, rel_dist, lscorer_rel.accuracy_score()))

    logger.debug('{:20}: ACC{} = {:.2%}'.format(ref_pm.source, rel_dist, gscorer_rel.accuracy_score()))

    return gscorer_rel, gscorer_abs
